...
class Widget extends React.Component {
    ...
    render() {
      let img = this.state.widgetData==null?null: Array.from(this.state.widgetData.keys());
       return(
        <div>
        {this.state.widgetData && (<div style={{position: 'relative',
    margin: 'auto',
  overflow: 'hidden',
  width: '520px',
  height: '350px',
  ...
  top: '10%',
  left: '60%'}}>
          <a href={this.state.userProfile}><p style={{fontSize: '0.6em',
  color: '#ba7e7e',
  letterSpacing: '1px'}}>{this.state.widgetData.get(img[0]).username}</p></a>
          <h1 style={{fontSize: '1.2em',
  color: '#4e4e4e',
  marginTop: '-5px'}}>{this.state.widgetData.get(img[0]).title}</h1>
          <h2 style={{color: '#c3a1a0',
  marginTop: '-5px',}}>{this.state.widgetData.get(img[0]).buyPrice} ETH</h2>
  ...
  letterSpacing: '1px',
  color: '#f5f5f5',
  cursor: 'pointer'}} >Buy Now</button></a>
          </div>
        </div>
      </div>
